...
// Для отключения предупреждений при использовании assert с неиспользуемыми переменными
#ifdef _MSC_VER
...
namespace {
    ...
    // "Магическое" число для отслеживания живости объекта
    inline constexpr uint32_t DEFAULT_COOKIE = 0xdeadbeef;

    // Тестовый объект для проверки "живости"
    struct TestObj {
        TestObj() = default;
    ...
    // Объект для подсчёта вызовов конструкторов/деструкторов
    struct Obj {
        Obj() {
    ...
    struct C {
        ...
        C& operator=(const C& other) noexcept {
            if (this != &other) ++copy_assign;
            return *this;
        }
        C& operator=(C&&) noexcept { ++move_assign; return *this; }
        ~C() { ++dtor; }

        static void Reset() { def_ctor = copy_ctor = move_ctor = copy_assign = move_assign = dtor = 0; }
    ...
    void Benchmark() {
        ...
        try {
            ...
            C c;
            {
                ...
                vector<C> v(NUM);
                v.push_back(c);
                cerr << "Default ctors: " << C::def_ctor << endl;
                cerr << "Move ctors: " << C::move_ctor << endl;
                cerr << "Copy ctors: " << C::copy_ctor << endl;
                cerr << "Dtors: " << C::dtor << endl;
            }
        }
        catch (...) {}
...
// Основная функция — чистая и понятная
int main() {
    try {
        Test1_BasicFunctionality();
        Test2_ExceptionSafety();
        Test3_MoveSemantics();
        Test4_PushBackAndResize();
        Test5_EmplaceBack();
        Test6_InsertEmplaceErase();
        Benchmark();
    }
