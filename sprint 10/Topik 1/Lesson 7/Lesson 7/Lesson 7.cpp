...
public:
	...
	std::shared_ptr<Value> GetValue(const Key& key) {
		...
		if (it != objects_.end()) {
			// Если значение найдено, проверяем, не удалено ли оно
			auto value = it->second.lock();
			if (value) {
				// Если значение не удалено, возвращаем его
				return value;
			}
		}

		// Если значение не найдено или удалено, создаём новое значение с помощью фабрики
...
private:
	std::unordered_map<Key, std::weak_ptr<Value>> objects_; // Контейнер закэшированных значений
	ValueFactoryFn value_factory_;                          // Фабрика создания значений
};
