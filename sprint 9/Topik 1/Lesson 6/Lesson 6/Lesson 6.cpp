...
struct PlateHasherAll {
    size_t operator()(const VehiclePlate& plate) const {
        ...
        auto letters_hash = (letters[0] - 'A') * 1'000'000ULL
            + (letters[1] - 'A') * 100'000'000ULL
            + (letters[2] - 'A') * 10'000'000'000ULL;

        return static_cast<size_t>(plate.GetDigits() + plate.GetRegion() * 1000 + letters_hash);
    }
};
