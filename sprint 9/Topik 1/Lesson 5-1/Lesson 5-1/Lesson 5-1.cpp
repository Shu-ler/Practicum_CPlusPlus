...
public:
	...
	// Возвращает представление номерного знака в виде форматированной строки
...
public:
	...
	void Park(VehiclePlate car) {
		// Используем оператор [] для доступа к элементу в контейнере.
		// Если элемент с таким ключом ещё не существует, он будет добавлен с значением по умолчанию (0).
		// Затем увеличиваем количество парковок на 1.
		++car_to_parks_[car];
	}
	...
	int GetCount(const VehiclePlate& car) const {
		// Для незарегистрированного автомобиля (car_to_parks_.count(car) == 0) возвращается 0
		// Для зарегистрированного - количество парковок
